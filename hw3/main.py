...
def warp_image(igs_in, igs_ref, H):
    ...
    i_indices = np.repeat(np.arange(height_ref), width_ref)
    j_indices = np.tile(np.arange(width_ref), height_ref)
    ...
    for i in range(height_ref):
        for j in range(width_ref):
            index_before_warp = indices_before_warp[i*width_ref+j]
            ...
            if(0<=i_before_warp<height_in and 0<=j_before_warp<width_in):
                igs_warp[i,j] = igs_in[i_before_warp,j_before_warp]

    # calculate igs_merge
    corners = warp_points(np.asarray([[0,0],[0,width_in-1],[height_in-1,0],[height_in-1,width_in-1]]),H)
    warp_end_height, warp_end_width = corners.max(axis = 0)
    warp_start_height, warp_start_width = corners.min(axis = 0)
    ...
    i_indices = np.repeat(np.arange(warp_start_height,warp_end_height), warp_end_width-warp_start_width)
    j_indices = np.tile(np.arange(warp_start_width,warp_end_width), warp_end_height-warp_start_height)
    indices = np.concatenate((i_indices.reshape(-1,1), j_indices.reshape(-1,1)),axis=1)

    indices_before_warp = warp_points(indices, H_inv)
    
    for i in range(warp_end_height - warp_start_height):
        for j in range(warp_end_width - warp_start_width):
            index_before_warp = indices_before_warp[i*(warp_end_width-warp_start_width)+j]
            i_before_warp = index_before_warp[0]
            j_before_warp = index_before_warp[1]
            if(0<=i_before_warp<height_in and 0<=j_before_warp<width_in):
                igs_merge[i+max(0,warp_start_height),j+max(0,warp_start_width)] = igs_in[i_before_warp,j_before_warp]
...
def set_cor_mosaic():
    # TODO ...
    p_in = np.asarray([[668,678],[675,691],[679,710],[655,958],[692,1066],[826,1342],[573,1363]], dtype=int)
    p_ref = np.asarray([[671,52],[679,68],[683,92],[656,388],[693,499],[817,744],[579,786]], dtype=int)
...
def set_cor_rec():
    ...
    c_in = np.asarray([[16,162],[27,275],[259,162],[245,275]], dtype=int)
    c_ref = np.asarray([[50,50],[50,200],[350,50],[350,200]], dtype=int)
